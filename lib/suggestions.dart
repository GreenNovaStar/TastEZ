      ...
      child: RefreshIndicator(
        ...
        child: Padding(
          ...
          child: ListView.builder(
            itemCount: widget.currUser.getSuggestCount() > 0 ? widget.currUser
                .getSuggestCount() : 0,
            itemBuilder: (context, i) {
              return Card(
                ...
                elevation: 0.2,
                child: ListTile(
                  title: (widget.response.data.recipes
                      .elementAt(i)
                      .title
                      .toString() != null) ? Text(
                      widget.response.data.recipes[i].title.toString()) : Text(
                      "PLACEHOLDER"),
                  leading: (widget.response.data.recipes[i].image.toString() !=
                      "" &&
                      widget.response.data.recipes[i].image.toString() != "null")
                      ? Image.network(
                      widget.response.data.recipes[i].image.toString())
                      : Image.asset('assets/nullimage.png'),
                  onTap: () => Navigator.push(context, MaterialPageRoute(
                      builder: (context) =>
                          recipePage(widget.currUser, widget.response.data
                              .recipes[i]))),
                  trailing: IconButton(
                    onPressed: () {
                      setState(() {
                        int indexOfFavoritedItem = inFavoriteList(
                            ...
                            widget.response.data.recipes
                                .elementAt(i)
                                .title
                                .toString());
                        if (indexOfFavoritedItem !=
                            -1) { //if item is in the favorite list,
                          if (widget.currUser.favorites[indexOfFavoritedItem]
                              .isFavorite) { //check if the item is favorited
                            widget.currUser.favorites[indexOfFavoritedItem]
                                .isFavorite = false; //then unfavorite it
                            widget.currUser.favorites.removeAt(
                    ...
                    ((inFavoriteList(widget.currUser.favorites, widget.response
                        .data.recipes
                        .elementAt(i)
                        .title
                        .toString())) != -1 ? Icon(Icons.favorite_rounded) : Icon(
